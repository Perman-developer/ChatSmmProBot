...
async def accept_pay(callback: CallbackQuery):
    try:
        ...
        # Xabar matni bo'lsa tahrirla, bo'lmasa yangi xabar yubor
        try:
            if callback.message.text:
                await callback.message.edit_text("To'lov qabul qilindi", reply_markup=None)
            else:
                await callback.message.answer("To'lov qabul qilindi")
        except Exception:
            # Xabarni tahrirlash xatolikka uchrasa, yangi xabar yubor
            await callback.message.answer("To'lov qabul qilindi")
